...
public class TarantoolCallClient extends DB{
	...
	public int insert(String table, String key,
        ...
        tuple.setLuaArgument(0, space).setLuaString(1, key, "UTF-8");
		for (Map.Entry<String, ByteIterator> i: values.entrySet()){
			tuple.setLuaString(2 * (j + 1), i.getKey(), "UTF-8").setLuaString(2 * (j + 1) + 1, 
	...
	public int read(String table, String key, Set<String> fields,
		...
		Tuple tuple = new Tuple(3).setLuaArgument(0, space).setLuaArgument(1, 0).setLuaString(2, key, "UTF-8");
		Tuple resp;
		try {
	...
	public int delete(String table, String key) {
		try{
			connection.call(space, "box.delete", new Tuple(2).setLuaArgument(0, space).setLuaString(1, key, "UTF-8"));
		} catch (TarantoolException e){
			e.printStackTrace();
			return 1;
		}
